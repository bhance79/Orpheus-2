...
from flask import Flask, redirect, request, session, url_for, render_template, flash, jsonify, Response, stream_with_context
...
# ---------- USB Pod ----------
@app.route('/api/usbpod/browse-folder', methods=['GET'])
...
@app.route('/api/usbpod/download', methods=['POST'])
def api_usbpod_download():
    ...
    # Fetch all tracks from Spotify before streaming
    try:
        items = list(paginate(lambda o, l: sp.playlist_items(playlist_id, limit=l, offset=o)))
    except Exception as e:
